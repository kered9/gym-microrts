...
#seeds to run: 3, 5, 12; seeds run: 1

import argparse
import discounts as df
import os
import random
import time
import subprocess
...
def parse_args():
    ...
    parser.add_argument('--wandb-project-name', type=str, default="ppo-ctf",
        help="the wandb's project name")
    parser.add_argument('--wandb-entity', type=str, default='dmartin7',
    ...
    # Discount specific arguments
    parser.add_argument('--df', type=int, default=0,
        help="the discount function to use")
    parser.add_argument('--scale', type=float, default=0,
        help="the scale factor for hyperbolic discounting")
    parser.add_argument('--dynamic', type=bool, default=False,
        help="Toggle dynamic discount value")
    parser.add_argument('--increase', type=bool, default=False,
        help="Toggle increasing gamma value")
...
def increasing_gamma(start_e: float, end_e: float, duration: int, t: int):
    slope =  (end_e - start_e) / duration
    return min(slope * t + start_e, end_e)

class Agent(nn.Module):
    def __init__(self, envs, mapsize):
        ...
        self.actor = nn.Sequential(
            ...
            nn.ReLU(),
            layer_init(nn.ConvTranspose2d(64, 78, 3, stride=3, padding=2, output_padding=1)),
            #for 16x16
...
if __name__ == "__main__":
    ...
    envs = MicroRTSGridModeVecEnv(
        ...
        map_paths=["maps/CTF/ctf.xml"],
        reward_weight=np.array([100.0, 0.0, 100.0, 0.0, 1.0, 0.0, 0.1]),
    )
        
    #win, gather resource, produce worker, produce building, attack, produce attack unit
    #win, grab flag, return flag, produce worker, attack holder, attack, holding flag, flag holder distance
    ...
    mapsize = 9*9
    agent = Agent(envs, mapsize).to(device)
    ...
    for update in range(starting_update, args.num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            for info in infos: 
                if "episode" in info.keys():
                    if args.dynamic:
                        print(f"global_step={global_step}, episode_reward={info['episode']['r']}, gamma={factor.mean()}")
                    else:
                        print(f"global_step={global_step}, episode_reward={info['episode']['r']}, gamma={factor}")
        ...
        with torch.no_grad():
            ...
            if args.gae:
                ...
                for t in reversed(range(args.num_steps)):
                    if args.dynamic:
                        factor = torch.exp(logprobs[t])                      
                    elif args.increase:
                        factor = increasing_gamma(args.gamma, 0.99, args.total_timesteps, global_step)                    
                    else:
                        factor = args.gamma
                    ...
                    if args.df == 0:
                        delta = rewards[t] + factor * nextvalues * nextnonterminal - values[t]
                        advantages[t] = lastgaelam = delta + factor * args.gae_lambda * nextnonterminal * lastgaelam
                    elif args.df == 1:
                        delta = rewards[t] + df.exponential(nextvalues, t, 1-factor) * nextnonterminal - values[t]
                        advantages[t] = lastgaelam = delta + df.exponential(args.gae_lambda * lastgaelam, t, 1-factor) * nextnonterminal
                    elif args.df == 2:
                        delta = rewards[t] + df.hyperbola(nextvalues, t, 1-factor, args.scale) * nextnonterminal - values[t]
                        advantages[t] = lastgaelam = delta + df.hyperbola_d(args.gae_lambda * lastgaelam, t, 1-factor, args.scale) * nextnonterminal                    
                ...
                for t in reversed(range(args.num_steps)):
                    ...
                    if args.df == 0:
                        returns[t] = rewards[t] + factor * nextnonterminal * next_return
                    elif args.df == 1:
                        returns[t] = rewards[t] + df.exponential(next_return, t, 1-factor) * nextnonterminal
                    elif args.df == 2:
                        returns[t] = rewards[t] + df.hyperbola_d(next_return, t, 1-factor, args.scale) * nextnonterminal                    
        ...
        if args.dynamic:
            writer.add_scalar("charts/gamma", factor.mean(), global_step)
        else:
            writer.add_scalar("charts/gamma", factor, global_step)
